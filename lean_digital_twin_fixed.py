...
class SelectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, title="Select Items", item_list=None):
...
class LeanDigitalTwin(tk.Tk):
    ...
    def _show_context_menu(self, event):
        ...
        if item_type == 'root':
            ...
            context_menu.add_command(label="Create Assets", command=self._create_asset)
    ...
    def _create_from_repository(self, node_tag):
        """Simulates fetching from a repository and adding selected items."""
        # --- SIMULATED REPOSITORY FETCH ---
        # In a real application, this would be a database call or API request
        repo_nodes = [
            'Pump-101A', 'Motor-202B', 'Valve-303C', 'Compressor-404D',
            'HeatExchanger-505E', 'Vessel-606F', 'Conveyor-707G'
        ]

        dialog_title = "Select Equipment" if node_tag == 'equipment_node' else "Select Sub-Equipment"
        dialog = SelectionDialog(self, title=dialog_title, item_list=repo_nodes)

        # This makes the main code wait until the dialog is closed
