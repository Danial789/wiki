...
class LeanDigitalTwin(tk.Tk):
    ...
    [VERSION: FINAL - Standard Copy/Paste]
    """
    ...
    def __init__(self):
        ...
        self.app_clipboard = None  # Add app clipboard attribute
    ...
    def _build_graphdb_tab(self, parent):
        ...
        node_select_frame = ttk.LabelFrame(parent, text="2. Master Node List (Double-click to set Active Node manually)", padding=10)
        ...
        ttk.Button(node_select_frame, text="Fetch All Nodes from Repo", command=self._fetch_nodes).grid(row=0, column=0, sticky="w", padx=5, pady=5)
    ...
    def _build_datasheet_editor_tab(self, parent):
        ...
        self.tag_text_embed = tk.Text(info_frame, height=4, width=30, font=('Segoe UI', 10), relief=tk.SOLID, borderwidth=1)
        ...
        ttk.Label(info_frame, text="Associated Node Properties (Double-click to preview):", style='Header.TLabel').grid(row=3, column=0, sticky="w", pady=(10, 5))
        self.properties_text_embed = tk.Text(info_frame, height=8, width=30, font=('Segoe UI', 10), relief=tk.SOLID, borderwidth=1)
        ...
        paste_frame = ttk.LabelFrame(info_frame, text="Copy & Paste Tools", padding=10)
        paste_frame.grid(row=5, column=0, sticky="ew", pady=(10,0))
        paste_frame.columnconfigure(0, weight=1)
        
        self.live_value_button = ttk.Button(paste_frame, text="Copy Value: (none)")
        self.live_value_button.grid(row=0, column=0, sticky="ew", pady=(5,5))
        
        self.live_unit_button = ttk.Button(paste_frame, text="Copy Unit: (none)")
        self.live_unit_button.grid(row=1, column=0, sticky="ew", pady=5)

        ttk.Separator(paste_frame, orient='horizontal').grid(row=2, column=0, sticky='ew', pady=10)

        self.paste_button = ttk.Button(paste_frame, text="Paste from Clipboard", command=self._paste_from_clipboard)
        self.paste_button.grid(row=3, column=0, sticky="ew", pady=5)
    ...
    def _update_data_model(self, event=None):
        ...
        def task():
            node_conditions = " || ".join([f"sameTerm(?subject, ex:{node}) || sameTerm(?object, ex:{node})" for node in nodes])
    ...
    def _draw_graph(self):
        ...
        try:
            ...
            nx.draw(self.graph, pos, ax=ax, with_labels=True, node_color='#a0cbe2', node_size=2500, font_size=10, font_weight='bold', width=1.5, edge_color='gray', arrows=True)
    ...
    def _select_property(self, event=None):
        ...
        selected_property = self.prop_listbox_selected.get(self.prop_listbox_selected.curselection())
        self.entry_prop.delete(0, tk.END)
        self.entry_prop.insert(0, selected_property)
        self.entry_val.focus_set()
        
    def _add_property(self):
        prop = self.entry_prop.get().strip()
        val = self.entry_val.get().strip().upper()
        unit = self.entry_unit.get().strip().upper()
        
        if not prop or not val:
            messagebox.showwarning("Input Required", "Property and Value Cell are required.")
            return

        self._update_or_add_mapping(prop, val, 'value')
        if unit:
            self._update_or_add_mapping(prop, unit, 'unit')

        self._refresh_property_listboxes()
        
        for entry in [self.entry_prop, self.entry_val, self.entry_unit]:
            entry.delete(0, tk.END)
        
    def _refresh_property_listboxes(self):
    ...
    def _write_to_excel(self):
        if not self.current_excel_path or not self.properties or not self.active_node:
            messagebox.showerror("Missing Data", "Ensure an Active Node is set, a target datasheet is chosen, and all properties are mapped.")
            return
        self._update_status("Writing all mappings to Excel...")
        ...
        def task():
            try:
                ...
                for prop, val_cell, unit_cell in self.properties:
                    ...
                    val, uni = (res_bnode[0]["value"]["value"], res_bnode[0].get("unit", {}).get("value")) if res_bnode else (None, None)
    ...
    def _on_property_double_click(self, event=None):
        try:
            index = self.properties_text_embed.index(f"@{event.x},{event.y}")
            line_start = self.properties_text_embed.index(f"{index} linestart")
            line_end = self.properties_text_embed.index(f"{index} lineend")
            prop = self.properties_text_embed.get(line_start, line_end).strip()
            ...
            if not self.active_node:
                ...
                messagebox.showinfo("Info", "An active node must be set to see a live value preview. Select a tag with one node or choose a node manually in the 'GraphDB → Excel' tab.")
                return
            ...
            def task():
                ...
                def update_ui():
                    self.live_value_button.config(text=f"Copy Value: {val or '(none)'}", command=lambda v=val: self._copy_to_app_clipboard(v))
                    self.live_unit_button.config(text=f"Copy Unit: {uni or '(none)'}", command=lambda u=uni: self._copy_to_app_clipboard(u))
    ...
    def _set_active_node(self, node_name):
        ...
        if node_name:
            ...
            self.active_node_display_label.config(text="None (Select a tag with one node, or choose from list)")
            self.node_listbox.selection_clear(0, tk.END)
            
    def _execute_single_mapping(self, event=None):
        if not event.widget.curselection(): return

        if not self.active_node:
            messagebox.showerror("Node Not Selected", "Please set an Active Node before executing a mapping.")
            return

        if not self.current_excel_path:
            messagebox.showerror("Datasheet Not Open", "Please open a datasheet in the editor before executing a mapping.")
            return

        node = self.active_node
        
        try:
            full_string = event.widget.get(event.widget.curselection())
            prop = full_string.split('→')[0].strip()
            
            import re
            val_match = re.search(r"Val:\s*([A-Z0-9]+)", full_string)
            unit_match = re.search(r"Unit:\s*([A-Z0-9]+)", full_string)
            
            val_cell = val_match.group(1) if val_match else None
            unit_cell = unit_match.group(1) if unit_match else None

            if not val_cell:
                raise ValueError("Could not parse value cell from mapping.")
                
        except Exception as e:
            messagebox.showerror("Parsing Error", f"Could not parse the selected mapping string:\n{e}")
            return

        self._update_status(f"Executing mapping for {prop}...")
        ...
        def task():
            try:
                ...
                self.after(0, lambda: messagebox.showerror("Execution Error", f"An error occurred: {e}"))
                self.after(0, lambda: self._update_status("Error executing mapping.", 4000))
        
        threading.Thread(target=task, daemon=True).start()
        
    def _copy_to_app_clipboard(self, value_to_copy):
        if value_to_copy is None:
            messagebox.showinfo("No Value", "There is no value to copy.")
            return
        
        self.app_clipboard = value_to_copy
        self._update_status(f"'{value_to_copy}' copied. Select a cell and click 'Paste'.", 4000)

    def _paste_from_clipboard(self):
        """Paste from system clipboard to selected cell"""
        ...
        try:
            ...
            messagebox.showwarning("Clipboard Empty", "The system clipboard is empty.")
            return

        # Get selected cell using correct pandastable methods
        try:
            # Method 1: Try to get current cell position
            if hasattr(self.pt_table, 'currentrow') and hasattr(self.pt_table, 'currentcol'):
                row = self.pt_table.currentrow
                col = self.pt_table.currentcol
            else:
                # Method 2: Try alternative approach for getting selected cells
                if hasattr(self.pt_table, 'multiplerowlist') and self.pt_table.multiplerowlist:
                    row = self.pt_table.multiplerowlist[0]
                    col = 0  # Default to first column if column info not available
                elif hasattr(self.pt_table, 'startrow'):
                    row = self.pt_table.startrow
                    col = getattr(self.pt_table, 'startcol', 0)
                else:
                    messagebox.showwarning("No Cell Selected", "Please click a cell in the table to select it as the target.")
                    return
        ...
        try:
            ...
            self.pt_table.model.df.iloc[row, col] = clipboard_content
            self.pt_table.redraw()
            self._update_status(f"Pasted '{clipboard_content}'. Saving in background...")
            ...
            def save_task():
                try:
                    ...
                    sheet.cell(row=excel_row, column=excel_col).value = clipboard_content
                    workbook.save(abs_path)
                    self.after(0, lambda: self._update_status(f"Successfully saved paste of '{clipboard_content}'.", 4000))
                except Exception as e:
                    self.after(0, lambda: messagebox.showerror("Save Error", f"Failed to save the pasted value: {e}"))

            threading.Thread(target=save_task, daemon=True).start()

        except Exception as e:
            messagebox.showerror("Paste Error", f"Could not paste value: {e}")

    def _paste_from_app_clipboard(self):
        """Fixed version of the paste from app clipboard method"""
        if self.app_clipboard is None:
            messagebox.showwarning("Clipboard Empty", "Please copy a value first.")
            return
        ...
        # Fix: Use correct pandastable Table methods to get selected cell
        try:
            # Method 1: Try to get current cell position
            if hasattr(self.pt_table, 'currentrow') and hasattr(self.pt_table, 'currentcol'):
                row = self.pt_table.currentrow
        ...
        value_to_paste = self.app_clipboard

        try:
            # Update the view instantly
            self.pt_table.model.df.iloc[row, col] = value_to_paste
            self.pt_table.redraw()
            self._update_status(f"Pasted '{value_to_paste}'. Saving in background...")

            # Save non-destructively in the background
            def save_task():
                try:
                    abs_path = os.path.abspath(self.current_excel_path)
                    sheet_name = self.sheet_selector_combobox.get()
                    ...
                    # openpyxl is 1-based, pandastable is 0-based
                    excel_row = row + 1
                    excel_col = col + 1
                    
                    sheet.cell(row=excel_row, column=excel_col).value = value_to_paste
                    
                    workbook.save(abs_path)
                    self.after(0, lambda: self._update_status(f"Successfully saved paste of '{value_to_paste}'.", 4000))
                except Exception as e:
                    self.after(0, lambda: messagebox.showerror("Save Error", f"Failed to save the pasted value: {e}"))
