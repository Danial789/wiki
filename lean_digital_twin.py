...
class LeanDigitalTwin(tk.Tk):
    ...
    def _build_datasheet_editor_tab(self, parent):
        ...
        ttk.Label(paste_frame, text="1. Double-click a property above.", style='Info.TLabel').grid(row=0, column=0, sticky="w")
        ttk.Label(paste_frame, text="2. Click Copy to copy values.", style='Info.TLabel').grid(row=1, column=0, sticky="w")
        ttk.Label(paste_frame, text="3. Click Paste to paste into selected cell.", style='Info.TLabel').grid(row=2, column=0, sticky="w")
        
        self.live_value_button = ttk.Button(paste_frame, text="Copy Value: (none)")
        self.live_value_button.grid(row=3, column=0, sticky="ew", pady=(10,5))
        
        self.live_unit_button = ttk.Button(paste_frame, text="Copy Unit: (none)")
        self.live_unit_button.grid(row=4, column=0, sticky="ew", pady=5)
        ...
        self.paste_button.grid(row=5, column=0, sticky="ew", pady=5)
    ...
    def _add_property(self, source='main'):
        if source == 'main':
            prop = self.entry_prop.get().strip()
            val = self.entry_val.get().strip().upper()
            unit = self.entry_unit.get().strip().upper()
            entries = [self.entry_prop, self.entry_val, self.entry_unit]
        elif source == 'embed':
            prop = self.entry_prop_embed.get().strip()
            val = self.entry_val_embed.get().strip().upper()
            unit = self.entry_unit_embed.get().strip().upper()
            entries = [self.entry_prop_embed, self.entry_val_embed, self.entry_unit_embed]
    ...
    def _refresh_property_listboxes(self):
        # This method is no longer needed as there are no property listboxes to refresh
        pass
    ...
    def _on_property_double_click(self, event=None):
        try:
            ...
            def task():
                ...
                def update_ui():
                    self.live_value_button.config(text=f"Copy Value: {val or '(none)'}", command=lambda v=val: self._copy_to_clipboard(v))
                    self.live_unit_button.config(text=f"Copy Unit: {uni or '(none)'}", command=lambda u=uni: self._copy_to_clipboard(u))
    ...
    def _paste_from_clipboard(self):
        ...
        try:
            ...
            # Get the currently selected cell from the pandastable
            selected_row = self.pt_table.getSelectedRow()
            selected_col = self.pt_table.getSelectedColumn()
            
            if selected_row is None or selected_col is None:
                messagebox.showinfo("No Cell Selected", "Please select a cell in the datasheet to paste into.")
                return
            
            # Get the column name from the selected column index
            col_names = list(self.pt_table.model.df.columns)
            if selected_col >= len(col_names):
                messagebox.showerror("Invalid Selection", "Selected column is out of range.")
                return
            ...
            self._update_status(f"Pasted '{clipboard_content}' into cell [{selected_row}, {col_name}]", 3000)
